...
const Edit = () => {
  ...
  const {
    ...
    err: errFetch,
    data,
  } = useFetch(`${process.env.REACT_APP_API_URL}/${id}.json`);
  ...
  return (
    <div className="create">
      ...
      <div className="container m-auto px-10 mb-10">
        <h2 className="text-3xl text-primary font-black transition duration-500 ease-in-out mb-10">
          Create a new post
        </h2>
        {data && (
          <form
          ...
          >
            ...
            <input
              ...
              value={post.snippet}
              onChange={(event) => {
                setPost({ ...post, snippet: event.target.value });
              }}
              className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-primary"
            />
 ...
 */}
            ...
            <button
              className={`${
                loading || loadingFetch ? "cursor-not-allowed" : ""
